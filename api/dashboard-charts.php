...
//   dashboard-charts.php?type=beach-ranking&period=monthly|yearly
...
try { 
    if (isset($_GET['trend'])) { 
        echo json_encode([
            'success' => true, 
            'data' => getTrendData($db, $_GET['trend'])
        ]); 
        exit; 
    } 
    ...
    switch ($type) { 
        case 'most-visited': 
            echo json_encode([
                'success' => true, 
                'data' => getMostVisited($db)
            ]); 
            break; 

        case 'beach-ranking': 
            // NEW: Read the selected period from the Most Visited Beaches dropdown.
            $period = $_GET['period'] ?? 'monthly';

            echo json_encode([
                'success' => true, 
                'data' => getBeachRanking($db, $period)
            ]); 
            break; 
...
// - Monthly = visitors during the current month.
// - Yearly = visitors during the current year.
// - Combines walk-in visitors + Confirmed/Completed reservations.
// - Includes every participating (Active) beach.
// - Ranked highest-to-lowest.
// - Total participating beaches remains unchanged.

function getBeachRanking($db, $period = 'monthly') {

    // Only allow the two supported periods.
    if ($period !== 'yearly') {
        $period = 'monthly';
    }
    ...
    if ($period === 'yearly') {
        ...
        $rows = $db->query("
            SELECT 
                b.beach_name,
                COALESCE(w.walkin_total, 0) +
                COALESCE(r.reservation_total, 0) AS total_visitors
            FROM beaches b

            LEFT JOIN (
                SELECT 
                    beach_id,
                    SUM(total_visitors) AS walkin_total
                FROM walk_in_visits
            ...
            LEFT JOIN (
                SELECT 
                    beach_id,
                    SUM(num_visitors) AS reservation_total
                FROM reservations
                WHERE status IN ('Confirmed', 'Completed')
...
function getAgeDistribution($db) { 
    ...
    $w = $db->query(
        "SELECT $bracketCols FROM walk_in_visits"
    )->fetch(PDO::FETCH_ASSOC); 
...
function getTrendData($db, $period) { 
    ...
    if ($period === 'yearly') { 
        ...
        $rows = $db->query(" 
            SELECT period, SUM(cnt) AS total FROM ( 
                SELECT 
                    YEAR(visit_date) AS period, 
                    SUM(total_visitors) AS cnt 
                FROM walk_in_visits 
                ...
                SELECT 
                    YEAR(reservation_date) AS period, 
                    SUM(num_visitors) AS cnt 
                FROM reservations 
                WHERE status IN ('Confirmed', 'Completed') 
    ...
    $rows = $db->query(" 
        SELECT period, SUM(cnt) AS total FROM ( 
            SELECT 
                DATE_FORMAT(visit_date, '%Y-%m') AS period, 
                SUM(total_visitors) AS cnt 
            FROM walk_in_visits 
            GROUP BY period 
